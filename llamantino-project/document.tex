...
\begin{document}
	...
	\begin{itemize}
		...
		A given user query is mapped to a vector space - according to the chosen embedding model - then the most similar text is retrieved according to a defined similarity threshold (0.45 in our case). 
		
		\item \textbf{Context stacking}: Finally, the retrieved text is added above the user query marking it with 'context' key-word to help the model identify additional information. We also provided precise instruction regarding what input structure the model should expect in the system prompt.
	...
	We finally show below a brief conversation between human user and our RAG System.
